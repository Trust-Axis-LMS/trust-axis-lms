...
export function CoursesPageClient({ courses }: { courses: any[] }) {
  // Dynamically compute categories from actual database courses
  const uniqueCategories = Array.from(new Set(courses.map(c => c.category))).filter(Boolean).sort();
