...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = await prisma.contentPost.findUnique({ where: { slug, type: "ARTICLE", status: "PUBLISHED" }, select: { title: true, excerpt: true } }).catch(() => null);
  if (!post) return { title: "Article — Trust Axis" };
...
export default async function ArticlePostPage({ params }: Props) {
  const { slug } = await params;
  const post = await prisma.contentPost.findUnique({ where: { slug, type: "ARTICLE", status: "PUBLISHED" } }).catch(() => null);
        ...
        <div className="bg-[#F9FAFB] border-b border-[#F4F4F5] py-12">
          <div className="container mx-auto px-4 md:px-8 max-w-4xl">
            <div className="flex items-center gap-3 mb-4">
              {post.category && <span className="inline-block bg-black text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-sm">{post.category}</span>}
              {post.readTime && <span className="text-[12px] text-[#6C757D]">{post.readTime}</span>}
            </div>
            <h1 className="text-3xl md:text-5xl font-bold text-[#212529] tracking-tight leading-tight mb-4">{post.title}</h1>
            ...
            <div className="flex items-center gap-4 mt-6 pt-6 border-t border-[#E5E7EB]">
              ...
              {post.publishedAt && <span className="text-sm text-[#9CA3AF]">{new Date(post.publishedAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</span>}
            </div>
          </div>
        </div>
        <div className="container mx-auto px-4 md:px-8 max-w-6xl py-12 md:py-16">
          <div className="flex gap-12 items-start">
            <article className="flex-1 min-w-0">
              {post.content ? <TipTapRenderer content={post.content} /> : <p className="text-[#6C757D]">No content available.</p>}
            </article>
            {toc.length > 0 && (
              <aside className="hidden xl:block w-64 flex-shrink-0 sticky top-[90px]">
                <div className="border border-[#E5E7EB] rounded-xl p-5 bg-[#FAFAFA]">
                  ...
                  <ul className="space-y-1">
                    {toc.map((item) => (
                      <li key={item.id}>
                        <a href={`#${item.id}`} className="block text-[13px] text-[#6B7280] hover:text-[#007BFF] transition-colors py-1 leading-snug" style={{ paddingLeft: item.level === 2 ? "0" : `${(item.level - 2) * 12}px` }}>
                          {item.text}
                        </a>
                      </li>
                    ))}
